  ...
  class SubscriptionsController < Dashboard::BaseController
    ...
    def update
      ...
      unless Subscription::PLAN_CATALOG.key?(new_plan_type.to_sym)
        redirect_to dashboard_subscription_path(client_id: params[:client_id]), alert: t("meetia.dashboard.flash.invalid_plan")
        return
      end
      ...
      if @subscription&.trial? || new_plan_type != @target_client.subscription_plan
        ...
        redirect_to dashboard_subscription_path(client_id: params[:client_id]), notice: t("meetia.dashboard.flash.same_plan")
      end
    end

    def cancel_confirm
      unless cancellable_subscription
        redirect_to dashboard_subscription_path(client_id: params[:client_id]), alert: t("meetia.dashboard.flash.no_active_sub")
        return
      end
    ...
    def cancel
      unless cancellable_subscription
        redirect_to dashboard_subscription_path(client_id: params[:client_id]),
                    alert: t("meetia.dashboard.flash.sub_missing")
        return
      end
      ...
      begin
        ...
        if @subscription.update(status: :cancelled)
          ...
          redirect_to dashboard_subscription_path(client_id: params[:client_id]),
                      notice: t("meetia.dashboard.flash.cancel_done")
        else
          redirect_to dashboard_subscription_path(client_id: params[:client_id]),
                      alert: t("meetia.dashboard.flash.cancel_failed")
        end
        ...
        redirect_to dashboard_subscription_path(client_id: params[:client_id]),
                    alert: t("meetia.dashboard.flash.stripe_cancel_failed")
      end
    end
    ...
    def set_target_client
      if acting_as_admin?
        if params[:client_id].present?
          ...
          redirect_to dashboard_management_path, alert: t("meetia.dashboard.flash.client_required")
        end
      else
